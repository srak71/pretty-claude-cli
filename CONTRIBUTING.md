...
The status line reads session JSON on stdin. You can exercise it without a live
Claude Code session:

```bash
echo '{"context_window":{"used_percentage":75},
  "model":{"display_name":"Opus 4.8"},
  "workspace":{"current_dir":"'"$HOME"'/projects/demo"},
  "rate_limits":{"five_hour":{"used_percentage":40,"resets_at":'"$(($(date +%s)+7000))"'}}}' \
| PRETTY_CLAUDE_CONFIG=./config.example.sh bash statusline.sh; echo
```

Tweak the JSON (percentage, model, path) to see different states.

## What makes a good PR

- **Keep it config-driven.** Appearance belongs in `config.example.sh` /
  `render.py` reading `PCC_*` variables — not hard-coded. The script should stay
  editing-free for end users.
...
Themes are just a set of `PCC_*` values. To propose one, open a PR that adds a
short, commented block to the "Fun alternates" notes in `config.example.sh`
